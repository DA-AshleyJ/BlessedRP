...
function StartJob(rank)
    ...
    --})
    -- Added a new fucking trigger????? 
    --local content = string.format(Lang:t("info.message_text"), Config.Vehicles[rank][vehicleChoice].name, Config.Spawns[spawnLocation].name)
    --local phoneNumber = exports.high_phone:getPhoneNumber()
    --TriggerEvent("high_phone:receivedMessage", 0, content, attachments)
    QBCore.Functions.Notify(string.format(Lang:t("info.message_text"), Config.Vehicles[rank][vehicleChoice].name, Config.Spawns[spawnLocation].name), 'success')
        ...
        if inDestination then
            ...
            if jobActive then
                ...
                if Config.PayoutBasedOnDistance then
                    ...
                    TriggerServerEvent("hiype-cardelivery:server-add-money", math.random(Config.Destinations[destinationLocation].from, Config.Destinations[destinationLocation].to) + (extraPoints / 2.0) + rank * 10)
                end

...
Padlock.Close = function() {
    ...
    $.post('http://storeRobberies/PadLockClose');
}
...
Keypad.Close = function(data) {
	...
	if(data != null){
		if (data.error != null) {
			$.post('http://storeRobberies/CombinationFail');
		}
	}
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("http://storeRobberies/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('http://storeRobberies/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function unlock() {
    gameOver = true;
    $.post('http://storeRobberies/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1

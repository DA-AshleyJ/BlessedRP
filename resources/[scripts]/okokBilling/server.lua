...
local Webhook = 'https://discord.com/api/webhooks/978779047519744040/oLd8YmMKQXny9V3q7Lfy_t6OzvW3xRdhH8YLYosDV-zd_b1YxN4htDQey9ojJn3trFFQ'
...
AddEventHandler("okokBilling:PayInvoice", function(invoice_id)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			...
			TriggerEvent("qb-management:server:okokBillingDeposit", invoices.society, invoices.invoice_value)
					...
					if xPlayer == nil then
						MySQL.query('SELECT money FROM players WHERE citizenid = @id', {
							...
							MySQL.query('UPDATE players SET money = @playerAccount WHERE citizenid = @target', {
								...
								TriggerEvent("qb-management:server:okokBillingDeposit", v.society, invoice_value)

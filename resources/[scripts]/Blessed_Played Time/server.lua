...
function dropPlayer(source)
    ...
    if(playersData[identifier] ~= nil and playersDataActuall[identifier] ~= nil) then
        ...
        sendToDiscord('Logged out', message)
    ...
    print('Blessed_playtime didnt recognize player')
    end
end
...
function sendToDiscord(name, message, footer)
    if Config.WebhookLink ~= '' then
        ...
        PerformHttpRequest(Config.WebhookLink, function(err, text, headers) end, 'POST', json.encode({username = BlessedRP, embeds = embed}), { ['Content-Type'] = 'application/json' })
    else
...
RegisterNetEvent('Blessed_playtime:loggedIn')
AddEventHandler('Blessed_playtime:loggedIn', function(playerName)
    ...
    if playersData[identifier] ~= nil then
        ...
        TriggerClientEvent('Blessed_playtime:notif', _source, Config.Strings['welcome']..'\n'..Config.Strings['ptotaltime'].. totaltimeFormatted ..'\n'..string.format(Config.Strings['loggedin'], playersDataLogged[identifier]))
 else        
        playersDataActuall[identifier] = actuallTime
        ...
        TriggerClientEvent('Blessed_playtime:notif', _source, Config.Strings['welcome1st'])
    end
end)

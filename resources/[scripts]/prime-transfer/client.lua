...
RegisterNetEvent('qb-transfer:giveCar1', function(model, plate)
    ...
    local vehname = GetDisplayNameFromVehicleModel(hash):lower()
        TriggerEvent('vehiclekeys:client:SetOwner', QBCore.Functions.GetPlate(veh))
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('qb-transfer:saveCar1', 0, QBCore.Shared.Vehicles[vehname], veh, GetVehicleNumberPlateText(veh))
        else
            QBCore.Functions.Notify(Config.Language[Config.UseLanguage].giveCar, 'error')
        end     
    end)

...
RegisterNetEvent('Blessed:motel:client:sendDoorlockState')
AddEventHandler('Blessed:motel:client:sendDoorlockState', function(doorlocktable)
    for i=1, #pinkcage, 1 do
        pinkcage[i].locked = doorlocktable[i].locked
    end
end)

RegisterNetEvent('Blessed:motel:client:sendDoorlockState2')
AddEventHandler('Blessed:motel:client:sendDoorlockState2', function(doorid, lockstate)
                ...
                if doordistance <= 2.0 then
                    ...
                    if (doordistance <= 1.5 and currentmotel == j) then
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent('Blessed:motel:server:toggleDoorlock', j, not pinkcage[j].locked)
                            openHouseAnim()
                        end
                    else
                    end

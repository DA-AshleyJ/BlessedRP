...
RegisterServerEvent("qb-bossmenu:server:AddMoney")
AddEventHandler("qb-bossmenu:server:AddMoney", function(account, amount)
    if not Accounts[account] then
        Accounts[account] = 0
    end
    ...
    if Employee then
        if Player.PlayerData.job.grade.level >= Employee.PlayerData.job.grade.level then
            if Employee.Functions.SetJob("unemployed", '0') then
                ...
                exports.oxmysql:execute('DELETE FROM `player_jobs` WHERE `cid` = ? and `job` = ?', {target, job})
            else

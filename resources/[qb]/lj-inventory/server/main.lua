...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			elseif name == "crafttable" then
				secondInv.name = "crafttable"
				secondInv.label = other.label
				secondInv.maxweight = 900000
				secondInv.inventory = other.items
				secondInv.slots = 9
...
RegisterNetEvent('inventory:server:SaveInventory', function(type, id, data)
	if type == "trunk" then
	...
	elseif type == "stash" then
		if data.curcrafting then
			Stashes[id].isOpen = false
			local SStashItems = Stashes[id].items
			TriggerClientEvent('inventory:client:transferdatacrafting', -1, SStashItems)
		else
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount, curcraft)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-inventory:server:SaveStashItems', function(stashId, items) -- WM
    MySQL.Async.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
...
QBCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, true, function(source, args)
	...
	local box1 = math.random(111,999)
		...
		if amount > 0 then
			if itemData then
				...
				elseif itemData["name"] == 'fishicebox' then
			   		Player.Functions.AddItem('fishicebox', 1, nil, box1, {["quality"] = 100})
...
local function useDriversLicense(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
...
local function useIDCard(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    if type(item) == 'string' then
        item = Player.Functions.GetItemByName(item)
...
QBCore.Commands.Add('givelicense', 'Show Your license', {}, false, function(source)
    useDriversLicense(source, "driver_license")
end, 'user')

QBCore.Functions.CreateUseableItem("driver_license", function(source, item)
    useDriversLicense(source, item)

end)

QBCore.Functions.CreateUseableItem("id_card", function(source, item)
    useIDCard(source, item)

end)

QBCore.Commands.Add('giveid', 'Show Your ID', {}, false, function(source)
    useIDCard(source, "id_card")
end, 'user')

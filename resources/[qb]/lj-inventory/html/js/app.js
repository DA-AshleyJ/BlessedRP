...
var  curcrafting = false; // WM ADDED
...
$(document).on("click", "#item-craft", function(e) { // WM ADDED
    $.post("https://lj-inventory/CraftItem") 
    e.preventDefault();
    Inventory.Close();
});
...
function FormatItemInfo(itemData, dom) {
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-description").html(
                '<p><strong>Box Owner: </strong><span>' + 
                itemData.info.boxOwner + '</span></p> Ice Box to store all of your fish'
            );
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        if (data.other != null && data.other.name == 'stash-Crafting_Table') { // WM ADDED
            curcrafting = true
            $("#item-craft").fadeIn(100)
        } else {
            curcrafting = false
            $("#item-craft").fadeOut(100)
        }
        ...
        var secinvslots = null;
        if (data.other != null && data.other != "") {
            if (curcrafting == true) {
                secinvslots = 15
            } else {
                secinvslots = data.other.slots
            }
            for (i = 1; i < secinvslots + 1; i++) { // WM
                ...
                if (item != null) {
                    ...
                    if (item.price != null) {
                        ...
                        $(".other-inventory")
                            .find("[data-slot=" + item.slot + "]")
                            .html(
                                '<div class="item-slot-img"><img src="images/' + // WM
                                item.image +
                                '" alt="' +
                                item.name +
                                '" /></div><div class="item-slot-amount"><p>' +
    ...
    Inventory.Close = function() {
        ...
        $.post("https://lj-inventory/CloseInventory", JSON.stringify({curcrafting}));

	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("core_dispatch:addCall", "10-10", "Fight In Progress", {{icon = "fa-solid fa-person", info = "Automatic Call"},{icon = "fa-solid fa-taxi", info = "Priority 2"}}, {alertCoords[1], alertCoolrds[2], alertCoords[3]}, "police", 10000, 110, 79)
			else
...
AddEventHandler("TakeHostage:killHostage", function()
	...
	local alertCoords = GetEntityCoords(PlayerPedId())
	TriggerServerEvent("core_dispatch:addCall", "10-60", "Shots Fired", {{icon = "fa-solid fa-person", info = "Automatic Call"},{icon = "fa-solid fa-taxi", info = "Priority 2"}}, {alertCoords[1], alertCoolrds[2], alertCoords[3]}, "police", 10000, 110, 79)
end)

			...
			if #(pos - vector3(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z)) < 1.5 then
				if GetClockHours() >= 18 and GetClockHours() <= 23 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, "Coke Dealer:- Do you have someting to sell? Press ~g~E~w~")
						...
						DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, "Coke Dealer: You have nothing to sell, Fuck off!!!")
					end
				else
					DrawText3D(Config.CokeLocation.x, Config.CokeLocation.y, Config.CokeLocation.z, "Coke Dealer: I don't know what are you talking, come in the ~r~evening~w~, Now fuck off!!")
				end
			end
		end
		if not inRange then
